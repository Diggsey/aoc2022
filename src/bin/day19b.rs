...
fn main() {
    let result: i32 = INPUT.lines().take(3).map(|line| {
        let (_id, ore_ore, clay_ore, obs_ore, obs_clay, geo_ore, geo_obs) = scan_fmt!(line, "Blueprint {}: Each ore robot costs {} ore. Each clay robot costs {} ore. Each obsidian robot costs {} ore and {} clay. Each geode robot costs {} ore and {} obsidian.", i32, i32, i32, i32, i32, i32, i32).unwrap();
        let bp = Blueprint {
            recipes: [
                [ore_ore, 0, 0],
                [clay_ore, 0, 0],
        ...
        let num_geo = bp.solve([0,0,0], [1,0,0], 31, None, &mut HashMap::new());
        println!("{}", num_geo);
        num_geo
    }).product();
    println!("{}", result);
